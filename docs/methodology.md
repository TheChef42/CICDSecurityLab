...
Actual scanner output is the only source of coverage. A scenario is covered by a tool only when that tool emits at least one finding that maps to the vulnerable-side evidence for that scenario. If a finding cannot be mapped, it remains visible as `UNMAPPED`.
